...
| `current_entity_name` | string\|null | нет | Название детали, выбранной/открытой в AR-сцене. Передаётся классификатору как контекст. Обновляется клиентом при получении команды `highlight_entity`. |
...
| `info` | Информационный запрос — ответ из базы знаний или из общих знаний (включая оффтопик, приветствия, вопросы об ассистенте) |
...
`null` для `intent: info` и `unknown_command`.
...
| `increase_scale` | Увеличить / приблизить модель |
| `decrease_scale` | Уменьшить / отдалить модель |
| `reset_view` | Сбросить вид в исходное состояние (позиция, масштаб, поворот) |
| `play_animation` | Запустить анимацию |
| `highlight_entity` | Подсветить конкретную деталь модели |

#### Структуры `client_command` по типу
...
`highlight_entity` — единственная команда с дополнительными полями:
```json
{
  "type": "highlight_entity",
  "target_nodes": ["Bone_Antenna 1", "Bone_Antenna 2", "Bone_Antenna 3", "Bone_Antenna 4"]
...
`target_nodes` — список имён узлов из FBX/GLB сцены, которые нужно подсветить. Присутствует только если был передан `current_model_id` и сервер смог разрезолвить деталь. Если `target_nodes` отсутствует или пустой — деталь не найдена, клиент может проигнорировать команду или показать уведомление.
...
```json
{
  "query": "где лучше рыбачить?",
  "intent": "info",
  "client_command": null,
  "answer": "Это вне моей тематики — я рассказываю только про космонавтику...",
  "context_used": false
}
```
...
| `current_entity_name` | string\|null | Название выбранной детали в AR-сцене |
...
Распознавание речи без обработки запроса. Принимает аудиофайл, возвращает только транскрипт. Требует авторизации.
...
```json
{
  "transcript": "расскажи про антенну"
}
```

**Ответ 400:** пустой аудиофайл.

---

## Рекомендации модели

Ассистент может самостоятельно предлагать следующий шаг в конце ответа, например:

> «Хотите расскажу и покажу подробнее про антенну?»

Если пользователь отвечает согласием («да», «конечно», «давай»), сервер возвращает `intent: hybrid` с `client_command: highlight_entity` для соответствующей детали.

**Важно для клиента:** при получении команды `highlight_entity` клиент должен обновить `current_entity_name` на подсвеченную деталь, чтобы последующие запросы отправлялись с правильным контекстом.

```
Ассистент: "...Хотите расскажу и покажу подробнее про антенну?"
Пользователь: "да"
  ↓
intent: hybrid
client_command: { "type": "highlight_entity", "target_nodes": ["Bone_Antenna 1", ...] }
answer: "Подсвечиваю элемент. Антенная система Спутника-1..."
  ↓
Клиент: подсвечивает антенну + обновляет current_entity_name = "Антенна 1"
```
...
Метаданные моделей хранятся в таблице `spacecraft` базы данных PostgreSQL. При первом запуске сервер автоматически переносит данные из `registry.json` в БД (если таблица пустая). Поле `size_bytes` вычисляется динамически по файлу на диске.
